...
LFPQuantizer::LFPQuantizer(unsigned PaletteSize) : m_size(0), m_limit(PaletteSize), m_index(0)
{
...
FIBITMAP *LFPQuantizer::Quantize(FIBITMAP *dib, int ReserveSize, RGBQUAD *ReservePalette)
{
	...
	if (FreeImage_GetBPP(dib) == 24)
	{
		...
		const unsigned color = 0 | src_line[FI_RGBA_BLUE] << FI_RGBA_BLUE_SHIFT | src_line[FI_RGBA_GREEN] << FI_RGBA_GREEN_SHIFT | src_line[FI_RGBA_RED] << FI_RGBA_RED_SHIFT;
		if (color != last_color)
		{
			last_color = color;
			last_index = GetIndexForColor(color);
