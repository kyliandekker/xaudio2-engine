	...
	try
	{
		...
		for (unsigned y = 0; y < height; y++)
		{
			for (unsigned x = 0; x < width; x++)
			{
				...
				if (x > 0)
					divg[index] -= gx[index - 1];
				if (y > 0)
					divg[index] -= gy[index - pitch];
			}
		}
...
static FIBITMAP *tmoFattal02(FIBITMAP *Y, float alpha, float beta)
{
	...
	try
	{
		...
		if (!GradientPyramid(pyramid, nlevels, gradients, avgGrad))
			throw(1);
...
FreeImage_TmoFattal02(FIBITMAP *dib, double color_saturation, double attenuation)
{
	...
	try
	{
		...
		// not needed anymore
		FreeImage_Unload(Yin);
		Yin = NULL;
		FreeImage_Unload(Yout);
		Yout = NULL;
