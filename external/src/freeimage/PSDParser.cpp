...
psdThumbnail::psdThumbnail() : _Format(-1), _Width(-1), _Height(-1), _WidthBytes(-1), _Size(-1), _CompressedSize(-1), _BitPerPixel(-1), _Planes(-1), _dib(NULL), _owned(true)
{
}
...
int psdThumbnail::Read(FreeImageIO *io, fi_handle handle, int iResourceSize, bool isBGR)
{
	...
	else
	{
		...
		for (unsigned h = 0; h < (unsigned)_Height; ++h, dst_line_start -= dstLineSize)
		{ //<*** flipped
...
*/
static BOOL invertColor(FIBITMAP *dib)
{
...
bool psdParser::WriteImageData(FreeImageIO *io, fi_handle handle, FIBITMAP *dib)
{
	...
	case PSDP_COMPRESSION_NONE: // raw data
	{
		for (unsigned c = 0; c < nChannels; c++)
		{
			...
			for (unsigned h = 0; h < nHeight; ++h, src_line_start -= srcLineSize)
			{ //<*** flipped
...
bool psdParser::Save(FreeImageIO *io, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data)
{
	...
	if (!_headerInfo.Write(io, handle))
		return false;
