...
jpeg_freeimage_src(j_decompress_ptr cinfo, fi_handle infile, FreeImageIO *io)
{
	...
	if (cinfo->src == NULL)
	{
		cinfo->src = (struct jpeg_source_mgr *)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_PERMANENT, sizeof(SourceManager));
...
jpeg_freeimage_dst(j_compress_ptr cinfo, fi_handle outfile, FreeImageIO *io)
{
	...
	if (cinfo->dest == NULL)
	{
		cinfo->dest = (struct jpeg_destination_mgr *)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_PERMANENT, sizeof(DestinationManager));
	}
	...
	if ((dib) && (handle))
	{
		try
		{
			...
			if (cinfo.in_color_space == JCS_RGB)
			{
				...
				else if ((flags & JPEG_SUBSAMPLING_422) == JPEG_SUBSAMPLING_422)
				{ // 2x1 (low)
				...
				else if ((flags & JPEG_SUBSAMPLING_444) == JPEG_SUBSAMPLING_444)
				{ // 1x1 (no subsampling)
