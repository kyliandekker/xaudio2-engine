	...
	try
	{
		...
		for (int i = 0; i < colors; i++)
		{
			...
			if (strstr(keys, " c "))
			{
				...
				while (*clr == ' ')
					clr++; // find the start of the hex rgb value
				if (*clr == '#')
				{
					...
					{
					case 3:
						n = sscanf(clr, "%01x%01x%01x", &red, &green, &blue);
						red |= (red << 4);
						green |= (green << 4);
						blue |= (blue << 4);
						break;
					case 6:
						n = sscanf(clr, "%02x%02x%02x", &red, &green, &blue);
						break;
					case 9:
						n = sscanf(clr, "%03x%03x%03x", &red, &green, &blue);
						red >>= 4;
						green >>= 4;
						blue >>= 4;
						break;
					case 12:
						n = sscanf(clr, "%04x%04x%04x", &red, &green, &blue);
						red >>= 8;
						green >>= 8;
						blue >>= 8;
						break;
	...
	if ((dib != NULL) && (handle != NULL))
	{
		...
		for (y = 0; y < height; y++)
		{
			...
			for (x = 0; x < width; x++)
			{
				...
				if (color2chrs.find(u.index) == color2chrs.end())
				{ // new color
