...
#define CONVERT(from, to)                                                             \
	case to:                                                                          \
		FreeImage_ConvertLine##from##To##to(bits, scanline, FreeImage_GetWidth(dib)); \
		break;

#define CONVERTWITHPALETTE(from, to)                                                                             \
	case to:                                                                                                     \
		FreeImage_ConvertLine##from##To##to(bits, scanline, FreeImage_GetWidth(dib), FreeImage_GetPalette(dib)); \
		break;

#define CONVERTTO32WITHPALETTE(from)                                                                                                                                                                        \
	case 32:                                                                                                                                                                                                \
		if (bIsTransparent)                                                                                                                                                                                 \
		{                                                                                                                                                                                                   \
...
FreeImage_ColorQuantizeEx(FIBITMAP *dib, FREE_IMAGE_QUANTIZE quantize, int PaletteSize, int ReserveSize, RGBQUAD *ReservePalette)
{
	...
	if (ReserveSize < 0)
		ReserveSize = 0;
	if (ReserveSize > PaletteSize)
		ReserveSize = PaletteSize;
	if (FreeImage_HasPixels(dib))
	{
