	...
	try
	{
		...
		if (page == 0)
		{
			...
			for (idx = 0; idx < info->application_extension_offsets.size(); idx++)
			{
				...
				io->read_proc(&b, 1, 1, handle);
				if (b == 11)
				{ // All AEs start with an 11 byte sub-block to determine what type of AE it is
					char buf[11];
					io->read_proc(buf, 11, 1, handle);
					if (!memcmp(buf, "NETSCAPE2.0", 11) || !memcmp(buf, "ANIMEXTS1.0", 11))
					{ // Not everybody recognizes ANIMEXTS1.0 but it is valid
						io->read_proc(&b, 1, 1, handle);
						if (b == 3)
						{									 // we're supposed to have a 3 byte sub-block now
...
Save(FreeImageIO *io, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data)
{
	...
	try
	{
		...
		packed = 0;
		if (!no_local_palette)
			packed |= GIF_PACKED_ID_HAVELCT | ((bpp - 1) & GIF_PACKED_ID_LCTSIZE);
		if (interlaced)
			packed |= GIF_PACKED_ID_INTERLACED;
		io->write_proc(&packed, 1, 1, handle);
