...
constexpr auto SMPL_CHUNK_ID = "smpl";
	...
	public:
		SMPL_Chunk();
		SMPL_Chunk(unsigned char a_ChunkID[CHUNK_ID_SIZE], uint32_t a_ChunkSize, uint32_t a_Manufacturer, uint32_t a_Product, uint32_t a_SamplePeriod, uint32_t a_MidiUnityNode, uint32_t a_MidiPitchFraction, uint32_t a_Smpte_Format, uint32_t a_Smpte_Offset, uint32_t a_NumSampleLoops, uint32_t a_SamplerData, SMPL_Sample_Loop *a_Samples);
	...
	*/
	...
	public:
		CUE_Point();
		CUE_Point(uint32_t a_Id, uint32_t a_Position, uint32_t a_DataChunkId, uint32_t a_ChunkStart, uint32_t a_BlockStart, uint32_t a_SampleOffset);
		CUE_Point(const CUE_Point &rhs);

		CUE_Point &operator=(const CUE_Point &rhs);

		uint32_t id = 0;
		uint32_t position = 0;
		uint32_t data_chunk_id = 0;
...
#pragma pack(pop)
	...
	public:
		...
		bool
			...
			filledCueChunk = false;
	};

	template <class T>
	uint32_t GetChunkSize(bool a_RemoveBaseChunk = true)
	{
		static_assert(std::is_base_of<Chunk, T>::value, "T must inherit from a Chunk and must have a GetSize static method.");
		return T::GetSize() - (a_RemoveBaseChunk ? sizeof(Chunk) : 0);
	}
}