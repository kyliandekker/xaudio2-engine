...
namespace uaudio
{
    ...
    protected:
        ...
        static void SetFACTChunk(WavFormat& a_WavFormat, unsigned char a_ChunkId[CHUNK_ID_SIZE], uint32_t a_ChunkSize, uint32_t a_SampleLength);
        static void SetSMPLChunk(WavFormat& a_WavFormat, unsigned char a_ChunkId[CHUNK_ID_SIZE], uint32_t a_ChunkSize, uint32_t a_Manufacturer, uint32_t a_Product, uint32_t a_SamplePeriod, uint32_t a_MidiUnityNode, uint32_t a_MidiPitchFraction, uint32_t a_SmpteFormat, uint32_t a_SmpteOffset, uint32_t a_NumSampleLoops, uint32_t a_SamplerData);
