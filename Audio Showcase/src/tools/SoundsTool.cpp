...
SoundsTool::SoundsTool(uaudio::AudioSystem &a_AudioSystem) : BaseTool(0, "Sounds", "Sounds"), m_AudioSystem(a_AudioSystem)
{
    m_AudioSystem.CreateSoundChannel(m_ChannelIndex);
}

void SoundsTool::Render()
{
    uaudio::utils::uaudio_vector<uaudio::UAUDIO_DEFAULT_HASH> vec;
    uaudio::SoundSys.GetSoundHashes(vec);
...
void SoundsTool::RenderSound(uaudio::UAUDIO_DEFAULT_HASH a_SoundHash)
{
    ...
    ImGui::Text("%s", a_WaveFile->m_FilePath.c_str());

    ImGui::NewLine();

    bool hasFmtChunk = false, hasDataChunk = false;
    a_WaveFile->HasChunk(hasFmtChunk, uaudio::FMT_CHUNK_ID);
    a_WaveFile->HasChunk(hasDataChunk, uaudio::DATA_CHUNK_ID);

    if (hasFmtChunk && hasDataChunk)
    {
        std::string play_button_text = std::string(PLAY) + " Play##Play_Sound_" + a_WaveFile->m_FilePath;
        if (ImGui::Button(play_button_text.c_str()))
        {
            auto& comp = m_AudioSystem.CreateComponent();
            comp.Play(a_SoundHash);
        }
    }

    ImGui::SameLine();
    std::string remove_sound_text = std::string(MINUS) + " Unload##Unload_Sound_" + a_WaveFile->m_FilePath;
    if (ImGui::Button(remove_sound_text.c_str()))
    {
        ...
        //        channel->RemoveSound();
        //}
        uaudio::SoundSys.UnloadSound(a_SoundHash);
        return;
    }

    ImGui::SameLine();
    std::string save_sound_text = std::string(SAVE) + " Save##Save_Sound_" + a_WaveFile->m_FilePath;
    if (ImGui::Button(save_sound_text.c_str()))
        SaveFile(a_WaveFile);

    ImGui::SameLine();
    bool isLooping = false;
	a_WaveFile->IsLooping(isLooping);
	...
	a_WaveFile->GetChunkFromData<uaudio::FMT_Chunk>(fmt_chunk, uaudio::FMT_CHUNK_ID);

    const std::string loop_options_text = "Loop Options##Loop_Options_Sound_" + std::to_string(a_SoundHash);
    if (hasFmtChunk && hasDataChunk && ImGui::CollapsingHeader(loop_options_text.c_str()))
    {
        // TODO: Int range slider.
        uint32_t start_position = 0;
        uint32_t end_position = 0;
        float start_position_float = 0.0f;
        float end_position_float = 0.0f;
    	a_WaveFile->GetStartPosition(start_position);
        a_WaveFile->GetEndPosition(end_position);
        start_position_float = static_cast<float>(start_position);
        end_position_float = static_cast<float>(end_position);

        ImGui::Text("%s", std::string(
                              uaudio::utils::FormatDuration(uaudio::utils::PosToSeconds(start_position, fmt_chunk.byteRate), true) +
                              "/" +
                              uaudio::utils::FormatDuration(uaudio::utils::PosToSeconds(end_position, fmt_chunk.byteRate), true))
                              .c_str());
        ...
        m_AudioSystem.GetSoundChannel(channel, m_ChannelIndex);
        a_WaveFile->GetChunkSize(size, uaudio::DATA_CHUNK_ID);
        if (ImGui::RangeSliderFloat(range_slider_sound_name_text.c_str(), &start_position_float, &end_position_float, 0.0f, static_cast<float>(size), ""))
        {
            const uint32_t start_position_int = static_cast<uint32_t>(start_position_float);
            const uint32_t end_position_int = static_cast<uint32_t>(end_position_float);
            channel->SetSound(*a_WaveFile, uaudio::SoundData());
            if (start_position_temp != start_position_int)
            {
                uint32_t new_start_position = start_position_int % static_cast<uint32_t>(buffer_size);
                new_start_position = start_position_int - new_start_position;
                new_start_position = uaudio::utils::clamp<uint32_t>(new_start_position, 0, size);
                channel->PlayRanged(new_start_position, static_cast<uint32_t>(buffer_size));
                a_WaveFile->SetStartPosition(new_start_position);
            }
            else if (end_position_temp != end_position_int)
            {
                uint32_t new_end_position = end_position_int % static_cast<uint32_t>(buffer_size);
                new_end_position = end_position_int - new_end_position;
                new_end_position = uaudio::utils::clamp<uint32_t>(new_end_position, 0, size);
                a_WaveFile->SetEndPosition(size);
                channel->PlayRanged(new_end_position, static_cast<uint32_t>(buffer_size));
        ...
        if (ImGui::Button(left_button_start_text.c_str(), ImVec2(25, 25)))
        {
            ...
            new_start_position = uaudio::utils::clamp<uint32_t>(new_start_position, 0, size);
            channel->SetSound(*a_WaveFile, uaudio::SoundData());
            channel->PlayRanged(new_start_position, static_cast<int>(buffer_size));
            a_WaveFile->SetStartPosition(new_start_position);
        }
        ...
        {
            uint32_t new_start_position = start_position_int + static_cast<int>(buffer_size);
            new_start_position = uaudio::utils::clamp<uint32_t>(new_start_position, 0, size);
            channel->SetSound(*a_WaveFile, uaudio::SoundData());
            channel->PlayRanged(new_start_position, static_cast<int>(buffer_size));
            a_WaveFile->SetStartPosition(new_start_position);
        ...
        if (ImGui::Button(left_button_end_text.c_str(), ImVec2(25, 25)))
        {
            ...
            new_end_position = uaudio::utils::clamp<uint32_t>(new_end_position, 0, size);
            channel->SetSound(*a_WaveFile, uaudio::SoundData());
            channel->PlayRanged(new_end_position, static_cast<int>(buffer_size));
            a_WaveFile->SetEndPosition(new_end_position);
        }
        ...
        {
            uaudio::DATA_Chunk data_chunk;
        	a_WaveFile->GetChunkFromData<uaudio::DATA_Chunk>(data_chunk, uaudio::DATA_CHUNK_ID);
            uint32_t pos = size;
    ...
    if (ImGui::CollapsingHeader(chunk_specific_info_text.c_str()))
    {
        ...
        size_t num_chunks = 0;
        a_WaveFile->GetNumberOfChunks(num_chunks);
        for (size_t i = 0; i < num_chunks; i++)
        {
            uaudio::WaveChunkData* data = nullptr;
            a_WaveFile->GetChunkFromData(data, i);
            if (uaudio::utils::chunkcmp(uaudio::FMT_CHUNK_ID, &reinterpret_cast<char*>(data->chunk_id)[0]))
            {
                const std::string chunk_fmt_text = "FMT##FMT_" + std::to_string(a_SoundHash) + " " + std::to_string(i);
                if (ImGui::CollapsingHeader(chunk_fmt_text.c_str()))
                {
                    ImGui::Indent(IMGUI_INDENT);
                    ShowValue("Chunk ID: ", uaudio::FMT_CHUNK_ID);
                    uint32_t size;
                    a_WaveFile->GetChunkSize(size, uaudio::FMT_CHUNK_ID);
            ...
            else if (uaudio::utils::chunkcmp(uaudio::DATA_CHUNK_ID, &reinterpret_cast<char*>(data->chunk_id)[0]))
            {
                const std::string WaveChunkData_text = "DATA##DATA_" + std::to_string(a_SoundHash) + " " + std::to_string(i);
            ...
            else if (uaudio::utils::chunkcmp(uaudio::ACID_CHUNK_ID, &reinterpret_cast<char*>(data->chunk_id)[0]))
            {
                const std::string chunk_acid_text = "ACID##ACID_" + std::to_string(a_SoundHash) + " " + std::to_string(i);
                if (ImGui::CollapsingHeader(chunk_acid_text.c_str()))
                {
                    ImGui::Indent(IMGUI_INDENT);
                    ShowValue("Chunk ID: ", uaudio::ACID_CHUNK_ID);
                    uint32_t size;
                    a_WaveFile->GetChunkSize(size, uaudio::ACID_CHUNK_ID);
                    ShowValue("Chunk Size: ", std::to_string(size).c_str());

                    uaudio::ACID_Chunk acid_chunk;
                    a_WaveFile->GetChunkFromData<uaudio::ACID_Chunk>(acid_chunk, uaudio::ACID_CHUNK_ID);
            ...
            else if (uaudio::utils::chunkcmp(uaudio::BEXT_CHUNK_ID, &reinterpret_cast<char*>(data->chunk_id)[0]))
            {
                const std::string chunk_bext_text = "BEXT##BEXT_" + std::to_string(a_SoundHash) + " " + std::to_string(i);
                if (ImGui::CollapsingHeader(chunk_bext_text.c_str()))
                {
                    ImGui::Indent(IMGUI_INDENT);
                    ShowValue("Chunk ID: ", uaudio::BEXT_CHUNK_ID);
                    uint32_t size;
                    a_WaveFile->GetChunkSize(size, uaudio::BEXT_CHUNK_ID);
                    ShowValue("Chunk Size: ", std::to_string(size).c_str());

                    uaudio::BEXT_Chunk bext_chunk;
                    a_WaveFile->GetChunkFromData<uaudio::BEXT_Chunk>(bext_chunk, uaudio::BEXT_CHUNK_ID);
            ...
            else if (uaudio::utils::chunkcmp(uaudio::FACT_CHUNK_ID, &reinterpret_cast<char*>(data->chunk_id)[0]))
            {
                const std::string chunk_fact_text = "FACT##FACT_" + std::to_string(a_SoundHash) + " " + std::to_string(i);
                if (ImGui::CollapsingHeader(chunk_fact_text.c_str()))
                {
                    ImGui::Indent(IMGUI_INDENT);
                    ShowValue("Chunk ID: ", uaudio::FACT_CHUNK_ID);
                    uint32_t size;
                    a_WaveFile->GetChunkSize(size, uaudio::FACT_CHUNK_ID);
                    ShowValue("Chunk Size: ", std::to_string(size).c_str());

                    uaudio::FACT_Chunk fact_chunk;
                    a_WaveFile->GetChunkFromData<uaudio::FACT_Chunk>(fact_chunk, uaudio::FACT_CHUNK_ID);
                    ShowValue("Sample Length: ", std::to_string(fact_chunk.sample_length).c_str());
                    ImGui::Unindent(IMGUI_INDENT);
                }
            }
            else if (uaudio::utils::chunkcmp(uaudio::CUE_CHUNK_ID, &reinterpret_cast<char*>(data->chunk_id)[0]))
            {
	            const std::string chunk_cue_text = "CUE##CUE_" + std::to_string(a_SoundHash) + " " + std::to_string(i);
	            if (ImGui::CollapsingHeader(chunk_cue_text.c_str()))
	            {
	                ImGui::Indent(IMGUI_INDENT);
	                ShowValue("Chunk ID: ", uaudio::CUE_CHUNK_ID);
	                uint32_t size;
	                a_WaveFile->GetChunkSize(size, uaudio::CUE_CHUNK_ID);
	                ShowValue("Chunk Size: ", std::to_string(size).c_str());

	                uaudio::CUE_Chunk cue_chunk;
	                a_WaveFile->GetChunkFromData<uaudio::CUE_Chunk>(cue_chunk, uaudio::CUE_CHUNK_ID);
	                ...
	                for (uint32_t i = 0; i < cue_chunk.num_cue_points; i++)
	                {
	                    ...
	                    if (ImGui::CollapsingHeader(chunk_cue_point_text.c_str()))
	                    {
                            ...
                            const std::string chunk_smpl_loop_button_start_text = "Set As Start Position##Start_Position_" + std::to_string(i) + "_CUE_" + std::to_string(a_SoundHash);
                            if (ImGui::Button(chunk_smpl_loop_button_start_text.c_str()))
                                a_WaveFile->SetStartPosition(cue_chunk.cue_points[i].sample_offset * (fmt_chunk.blockAlign / fmt_chunk.numChannels));

                            const std::string chunk_smpl_loop_button_end_text = "Set As End Position##End_Position_" + std::to_string(i) + "_CUE_" + std::to_string(a_SoundHash);
                            if (ImGui::Button(chunk_smpl_loop_button_end_text.c_str()))
                                a_WaveFile->SetEndPosition(cue_chunk.cue_points[i].sample_offset * (fmt_chunk.blockAlign / fmt_chunk.numChannels));
	                        ImGui::Unindent(IMGUI_INDENT);
	                    }
	                }
	                ImGui::Unindent(IMGUI_INDENT);
            ...
            else if (uaudio::utils::chunkcmp(uaudio::SMPL_CHUNK_ID, &reinterpret_cast<char*>(data->chunk_id)[0]))
            {
	            const std::string chunk_smpl_text = "SMPL##SMPL_" + std::to_string(a_SoundHash) + " " + std::to_string(i);
	            if (ImGui::CollapsingHeader(chunk_smpl_text.c_str()))
	            {
	                ImGui::Indent(IMGUI_INDENT);
	                ShowValue("Chunk ID: ", uaudio::SMPL_CHUNK_ID);
	                uint32_t size;
	                a_WaveFile->GetChunkSize(size, uaudio::SMPL_CHUNK_ID);
	                ShowValue("Chunk Size: ", std::to_string(size).c_str());

	                uaudio::SMPL_Chunk smpl_chunk;
	                a_WaveFile->GetChunkFromData<uaudio::SMPL_Chunk>(smpl_chunk, uaudio::SMPL_CHUNK_ID);
	                ...
	                for (uint32_t i = 0; i < smpl_chunk.num_sample_loops; i++)
	                {
	                    ...
	                    if (ImGui::CollapsingHeader(chunk_smpl_loop_text.c_str()))
	                    {
	                        ...
	                        if (ImGui::Button(chunk_smpl_loop_button_start_text.c_str()))
	                            a_WaveFile->SetStartPosition(smpl_chunk.samples[i].start * (fmt_chunk.blockAlign / fmt_chunk.numChannels));
	                        ...
	                        if (ImGui::Button(chunk_smpl_loop_button_end_text.c_str()))
	                            a_WaveFile->SetEndPosition(smpl_chunk.samples[i].end * (fmt_chunk.blockAlign / fmt_chunk.numChannels));
            ...
            {
	            const std::string chunk_other_text = std::string(&data->chunk_id[0], &data->chunk_id[0] + std::size(data->chunk_id)) + "##" + std::string(&data->chunk_id[0], &data->chunk_id[0] + std::size(data->chunk_id)) + " " + std::to_string(i);
	            if (ImGui::CollapsingHeader(chunk_other_text.c_str()))
	            {
	                ImGui::Indent(IMGUI_INDENT);
	                ShowValue("Chunk ID: ", &reinterpret_cast<char*>(data->chunk_id)[0]);
	                uint32_t size;
	                a_WaveFile->GetChunkSize(size, &reinterpret_cast<char*>(data->chunk_id)[0]);
	                ShowValue("Chunk Size: ", std::to_string(size).c_str());
	                ImGui::Unindent(IMGUI_INDENT);
	            }
            }
...
void SoundsTool::RenderPlot(uaudio::UAUDIO_DEFAULT_HASH a_SoundHash)
{
}
...
/// </summary>
void SoundsTool::SaveFile(uaudio::WaveFormat *a_WaveFile)
{
    OPENFILENAME ofn;
    TCHAR sz_file[260] = {0};
    ...
    if (GetSaveFileName(&ofn))
    {
        ...
        uaudio::WaveReader::SaveSound(path, *a_WaveFile);
        delete[] path;
    }
}