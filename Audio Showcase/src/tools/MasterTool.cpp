...
MasterTool::MasterTool(uaudio::AudioSystem &a_AudioSystem) : BaseTool(0, "Actions", "Master Actions"), m_AudioSystem(a_AudioSystem)
{
    ...
    m_ChunkIds.push_back({uaudio::FMT_CHUNK_ID, false, false});
    m_ChunkIds.push_back({uaudio::DATA_CHUNK_ID, false, false});
...
void MasterTool::OpenFile()
{
    ...
    if (GetOpenFileName(&ofn))
    {
        ...
        uaudio::utils::uaudio_vector<const char *> chunks;
        ...
        uaudio::UAUDIO_DEFAULT_HASH hash;
    	uaudio::SoundSys.LoadSound(hash, path, path, m_WaveConfig);
        delete[] path;
    }
}